...
- **Every business rule ships with a property test.** `prop_<invariant>`, with the invariant
  stated in a comment in the words a human would use. `Money::split_evenly` is the model.
  Example tests are `<subject>_<behaviour>`.
- **If the implementation must deviate from `ref/domain-api.md`, fix the doc in the same
  commit.** Silent divergence turns the reference into a liability.
- **The module graph stays acyclic** (`ref/domain-api.md` §15). `just acyclic` enforces it.
