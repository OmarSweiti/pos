...
| A checked-in definition of the three rulesets | the payloads were applied with `gh api`; `scripts/gh-protect.sh` still refuses and exits 3 because it predates the rulesets API. Until it is rewritten against that API with negative tests, the rulesets are live configuration nobody can diff or restore — issue #115 stays open for exactly this |
