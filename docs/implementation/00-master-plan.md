...
| `doc_sequence PRIMARY KEY (register_id, kind)` | earlier schema drafts | `PRIMARY KEY (scope_kind, scope_id, kind)`. Receipts and Z reports stay register-scoped; fiscal ICV is store-scoped. That is no longer a default that can drift: `0005` shipped `CHECK (scope_kind IN ('register','store'))` under `1.9.1` on 14 September 2026 as **#113**'s option 4 — freeze `store`, knowingly — and a committed migration is never edited. The authoritative namespace is still open at `2.7.0`, so **`2.7.4` must re-check merchant decision 6.9 before it allocates the first ICV**: until that first row exists, a wrong scope costs one forward-only migration and no data repair; after it, a repair on a sequence required to be gapless |
...
| `trusted_time_state.confidence`, `observed_skew_milli` | earlier schema drafts | Removed. Both are outputs of `clock_confidence(...)`, and a stored verdict can disagree with the inputs it was derived from. The table maps `ClockState` ([`ref/domain-api.md`](ref/domain-api.md) §3.2) onto columns, plus the shell-owned `boot_token` and `updated_at`, with `anomaly` flattened into `anomaly_kind` / `anomaly_by_ms` / `anomaly_at` |
| `tender_type` without `is_internal` | earlier schema drafts | `is_internal INTEGER NOT NULL DEFAULT 0 CHECK (is_internal IN (0,1))`. It is not implied by `is_cash_counted = 0`: `card` moves real value through a PSP and `cliq` over bank rails, while an internal tender moves no value outside the two documents it settles. `store_credit`'s value is still `⚠️ OPEN` in [`ref/schema.md`](ref/schema.md) §0005 |
...
| **The reference register itself** — the machine that becomes the lowest register-hardware row | before group 1.7, on the same order as the printers | the deferred half of `1.2.0`, and therefore **every** performance budget: `1.2.7`, `1.4.9`, `1.6.2`, `1.11.13` and the `1.12.3` measurement job | `just bench-gate` keeps refusing, [`01-conventions.md`](01-conventions.md) §7's five limits stay unmeasurable, and four budget microsteps arrive with no machine to run on. A laptop standing in for it is worse than the wait: it makes every budget a number about the wrong computer |
