...
**The eleven values are the plan's, transcribed, with no 5-fil row.** `every_denomination_is_a_whole_number_of_qirsh` ties the table to merchant decision 2.1's default ten-fil step. A store whose answer to 2.1 is a five-fil step changes that test and this table together, deliberately. Whether the 25-fil piece is in everyday circulation, and whether the 5-fil piece belongs, is B.5's standing *"Verify the store's actual coin practice with the merchant"*, and it is part of 2.1's provisioning confirmation.
...
**Tests:** `denominations_are_descending_and_complete` · `every_denomination_is_a_whole_number_of_qirsh` · `no_other_currency_borrows_the_dinar_denominations`
**Done when:** `cargo nextest run -p pos-domain denomination` exits zero, proving the JOD table is exactly 50, 20, 10, 5 and 1 dinar and 500, 250, 100, 50, 25 and 10 fils, largest first, that every value is a whole number of qirsh, and that no other currency is given the dinar's table.
