...
**Four rules are properties of the whole document rather than of any field**, so `validate()` carries them: an `Acknowledgement` drops the QR line (`hardware-and-receipts.md` §2.3), the cash-rounding line prints *"only when non-zero"* so `Some(zero)` is a different and illegal document, a `BuyerBlock` exists because a TIN was captured, and every `Money` on one document shares one `Currency`. The struct stays public-fielded as §13 specifies rather than hiding behind a fallible constructor; **1.7.3's entry point calls `validate` before laying anything out**, which is where the call becomes mandatory.
**Tests:** `a_non_fiscal_acknowledgement_cannot_carry_a_fiscal_block` · `a_buyer_block_without_a_tin_is_refused` · `a_zero_rounding_adjustment_is_absent_rather_than_zero` · `buyer_name_never_reaches_debug_output` · `merchant_phone_never_reaches_debug_output` · `a_masked_pan_is_deliberately_still_visible_in_debug` · `a_receipt_round_trips_through_canonical_json` · `every_doc_kind_is_covered_by_the_validator` · `prop_a_model_mixing_two_currencies_is_refused` · `prop_validate_accepts_every_consistent_model`
**Done when:** `cargo nextest run -p pos-domain receipt::` exits zero, proving that `ReceiptModel::validate` refuses a document that mixes two currencies, one that calls itself a non-fiscal acknowledgement while carrying a fiscal block, and one that records a zero cash-rounding line — and that no `Debug` rendering of a model reveals a buyer name or a merchant phone.
