...
**Scheduled in:** define the matrix before 1.6.1, because the immutable seed lands only in migration `0004` and a committed migration is never reopened. **The database half is deferred, because the tables it queries do not exist yet.** `role` and `role_capability` are created by `0004`, which is 1.6.1; until that lands, `cargo nextest run -p pos-db --test role_matrix` would query nothing, and a test that cannot run is worse than an absent one because a green row reads as coverage. This step builds and verifies the domain view — which is also what 1.6.1 writes its seed *from* — so the two microsteps stop pointing at each other. The microstep therefore remains partly open until that deferred gate lands.
**Files (current half):** `crates/pos-domain/src/permissions.rs` (new) · `crates/pos-domain/src/lib.rs` (module declaration and re-exports) · [`ref/domain-api.md`](ref/domain-api.md) (§8's declarations and §8.2's framing) · this file (this microstep's prose, `Tests:` line and split)
