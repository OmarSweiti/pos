...
**The entry point calls `ReceiptModel::validate` before laying anything out**, and refuses rather than rendering. 1.7.1 put four whole-document rules there — an acknowledgement carrying a fiscal QR, a zero cash-rounding line, a buyer block with no TIN, two currencies on one document — and deliberately left `ReceiptModel`'s fields public, as §13 specifies, so nothing forces the call except this line and the test that proves it.
