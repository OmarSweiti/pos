...
> ⚠️ **OPEN — blocks the `doc_sequence` half of this migration.** The "scoped `doc_sequence`" above carries unratified merchant decision 6.9 — the authoritative ICV namespace, one of register, store, income source, credential, or one TIN across stores. It lands in `doc_sequence.scope_kind`, and because migrations are forward-only and are never edited once committed, the `CHECK` this migration writes is uneditable afterwards; a later correction is a second migration plus, if the wrong scope has already issued counter values, a data repair on a sequence required to be gapless. `ref/schema.md`'s running default is `CHECK (scope_kind IN ('register','store'))`, which is an assumption and not an answer. Do not inherit it by transcription. Default until answered: none — choose deliberately among the four options in **#113** (answer from the official ISTD package; widen the `CHECK` to all five candidate scopes and constrain the choice in code; defer `doc_sequence` out of `0005`; or freeze `store` knowingly). Owner: `2.7.0` ratifies 6.9 via #69, on a timeline outside this project's control, so this microstep cannot wait for it. Source that settles it: the official ISTD business rules or a written ISTD E-Invoicing Directorate ruling.
