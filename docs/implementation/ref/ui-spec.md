...
The harnesses the middle three rungs need are inventoried in [`test-catalog.md`](test-catalog.md). One of them now exists: microstep 1.11.0 gave `apps/terminal` a `jsdom` environment, `renderWithProviders`, explicit cleanup and a working fake clock, so the component tests this section depends on are writable — and are still unwritten, owed by the microsteps the catalogue names against that row. The packaged-app driver is still missing entirely. **A harness is not a layer**: until those tests are written the screen's automated defence is still a grep, which is why the distinction is recorded here rather than discovered at the moment the first of them is due.
