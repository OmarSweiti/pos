...
- `trusted_time_state` persists the **last instant a server authenticated its clock to this register** (`last_trusted_at`) together with the **boot-monotonic counter reading captured at that same instant** (`monotonic_since_trust_ms`), plus the device reading that accompanied it (`device_at_trust`), the retained `anomaly_*` triple, and the E.6 `high_water` emission floor. That anchor pair — authenticated UTC projected forward by `monotonic_now_ms - monotonic_since_trust_ms` — and not the wall clock, is what decides whether a lease is live. Confidence itself is **computed** by `clock_confidence(state, device_now, monotonic_now_ms, policy)` and deliberately never stored, because a stored verdict can disagree with the inputs it was derived from. `high_water` is a timestamp guard only — never a trust anchor, and never a substitute for the owned sequences required by I-7 ([`domain-api.md`](domain-api.md) §3.2, [`schema.md`](schema.md) §0005).
