...
**`scripts/check-test-catalog.py`** closes that. Local `just lint` reconciles the real catalog, and `just guards` runs its `--self-test`; `ci.yml` runs both — the reconciliation on the `rust` job, because it reads nextest listings, and the `--self-test` with the other negative suites on `guards`. Seven assertions are mechanical:
...
| 86 | A manager approval is replayed against a second, larger refund | `a_handle_used_twice_is_refused`, `an_altered_amount_is_refused`, `a_different_sale_is_refused`, `a_different_actor_is_refused`, `a_consumed_handle_is_still_consumed_after_restart`, `an_expired_handle_is_refused`, `an_approval_before_issuance_is_refused`, `approval_debug_output_redacts_content_hash_and_nonce`, `a_stored_handle_round_trips_through_restore`, `a_malformed_stored_handle_is_refused`, `approval_handles_cannot_be_deserialized`, `the_effect_and_the_consumption_commit_together_or_not_at_all` | 1 | unit + integration |
...
| **A DOM component harness for `apps/terminal`** — `@testing-library/react` + `user-event` + `jsdom`, with fake timers. **Landed at 1.11.0**, with `renderWithProviders`, an `index.html`-derived document fixture, explicit cleanup, and a `jest`-global bridge without which every `await user.*()` under fake timers hangs | `scan_routes_while_search_focused` and `scan_burst_detected_over_typing` landed with 1.11.6; `every_action_reachable_without_a_mouse` and `latin_runs_inside_arabic_text_are_bidi_isolated` are still owed, by 1.11.11 and 1.11.12. The fake-timer bridge earned itself: the two scan cases are the first to depend on a clock that only advances when a test says so |
